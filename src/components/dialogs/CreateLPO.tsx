...
function CreateLPO({
  ...
  const [createJobApi , {data : res , isSuccess , error , isError}] = useCreateJobMutation()
  ...
  async function onSubmit(data: any) {
    const response = await createJobApi({data : {...data , rfq : rfq_info.rfq_id , payment_terms :{ "1": "New payment"}}});
    ...
    <Dialog open={isDialogOpen} onOpenChange={() => setIsDialogOpen(false)}>
      <DialogContent className=" overflow-x-scroll no-scrollbar border border-black rounded-lg w-[90%] max-h-[90%]  scroll-smooth lg:w-[1200px] md:w-[1200px]">
        ...
        <span className="text-sm text-gray-600">Name</span>
          <h5 className="font-semibold text-lg">{rfq_info.client_name ? rfq_info.client_name : rfq_info.name}</h5>
          ...
          <h5 className="font-semibold text-lg">{rfq_info.quotation_amount} AED</h5>
        </div>
